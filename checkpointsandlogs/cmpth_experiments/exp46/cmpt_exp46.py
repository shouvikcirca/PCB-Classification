...
class Model(nn.Module):
	def __init__(self):
		...
		self.ff = nn.Sequential(nn.LayerNorm(100),nn.Linear(100,100,bias = False))	
	...
	def forward(self, X):
		...
		k = self.key(x)
		v = self.value(x)
		kqdots = q.unsqueeze(1)*k.unsqueeze(2)
		kqsoftmax = nn.Softmax(dim = 2)(kqdots)
		softmaxedvalues = torch.matmul(kqsoftmax, v.unsqueeze(2)).squeeze(2)
		identity1 = y+softmaxedvalues
		feedforward = self.ff(identity1)
		attentionoutput = feedforward + identity1
		mlpoutput = self.mlp(attentionoutput)
...
for i in range(epochs):
	...
	if valloss < minvalloss:
		if len(os.listdir('cmpth_checkpoints/exp46/checkpoints/fold{}'.format(f))) > 0:
			os.system('rm cmpth_checkpoints/exp46/checkpoints/fold{}/*'.format(f))
		savepath = 'cmpth_checkpoints/exp46/checkpoints/fold{}/'.format(f)+ 'fold{}_'.format(f) + datetime.now().strftime("%d_%m_%Y___%H_%M_%S")+'.pth'
		torch.save(model, savepath)
			


wpath = 'cmpth_checkpoints/exp46/logs/fold{}/'.format(f)+'trainlossrecord.npy'
trainlossrecord = np.array(trainlossrecord)
np.save(wpath, trainlossrecord)

wpath = 'cmpth_checkpoints/exp46/logs/fold{}/'.format(f)+'vallossrecord.npy'
vallossrecord = np.array(vallossrecord)
np.save(wpath, vallossrecord)

wpath = 'cmpth_checkpoints/exp46/logs/fold{}/'.format(f)+'valaccrecord.npy'
