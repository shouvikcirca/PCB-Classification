...
def compound_dice_loss(ypred, ytrue, device):
	...
	#h = torch.Tensor([trueratio, falseratio]).float().to(device)
	#i = g*h
	j = t.sum()
	return j
...
#for params in model.layer.classifier.parameters():
...
for i in range(epochs):
	...
	for samples, labels in trainloader:
		...
		if trues.sum() > 0:
			...
			truetrainloss = celoss(truepred, truelabels)
		...
		if falses.sum() > 0:
			...
			falsetrainloss = celoss(falsepred, falselabels)
	...
	if valloss < minvalloss:
		if len(os.listdir('cmpth_checkpoints/exp2/checkpoints/fold{}'.format(f))) > 0:
			os.system('rm cmpth_checkpoints/exp2/checkpoints/fold{}/*'.format(f))
		savepath = 'cmpth_checkpoints/exp2/checkpoints/fold{}/'.format(f)+ 'fold{}_'.format(f) + datetime.now().strftime("%d_%m_%Y___%H_%M_%S")+'.pth'
		torch.save(model, savepath)
			


wpath = 'cmpth_checkpoints/exp2/logs/fold{}/'.format(f)+'trainlossrecord.npy'
trainlossrecord = np.array(trainlossrecord)
np.save(wpath, trainlossrecord)

wpath = 'cmpth_checkpoints/exp2/logs/fold{}/'.format(f)+'vallossrecord.npy'
vallossrecord = np.array(vallossrecord)
np.save(wpath, vallossrecord)

wpath = 'cmpth_checkpoints/exp2/logs/fold{}/'.format(f)+'valaccrecord.npy'
