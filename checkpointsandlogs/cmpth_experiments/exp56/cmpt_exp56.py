...
for i in range(epochs):
	...
	for samples, labels in trainloader:
		...
		with torch.no_grad():
			...
			#train_gt = torch.cat([train_gt, torch.argmax(labels, dim=1).int().cpu()totaltrainloss+=(lossfactor * nlogloss(pred, labels))
	...
	if valloss < minvalloss:
		if len(os.listdir('cmpth_checkpoints/exp56/checkpoints/fold{}'.format(f))) > 0:
			os.system('rm cmpth_checkpoints/exp56/checkpoints/fold{}/*'.format(f))
		savepath = 'cmpth_checkpoints/exp56/checkpoints/fold{}/'.format(f)+ 'fold{}_'.format(f) + datetime.now().strftime("%d_%m_%Y___%H_%M_%S")+'.pth'
		torch.save(model, savepath)
			


wpath = 'cmpth_checkpoints/exp56/logs/fold{}/'.format(f)+'trainlossrecord.npy'
trainlossrecord = np.array(trainlossrecord)
np.save(wpath, trainlossrecord)

wpath = 'cmpth_checkpoints/exp56/logs/fold{}/'.format(f)+'vallossrecord.npy'
vallossrecord = np.array(vallossrecord)
np.save(wpath, vallossrecord)

wpath = 'cmpth_checkpoints/exp56/logs/fold{}/'.format(f)+'valaccrecord.npy'
