...
model = Model()
model.load_state_dict(torch.load('resnext_with_temp_raw.pth'))
...
for i in range(epochs):
	...
	if valloss < minvalloss:
		if len(os.listdir('cmpth_checkpoints/exp23/checkpoints/fold{}'.format(f))) > 0:
			os.system('rm cmpth_checkpoints/exp23/checkpoints/fold{}/*'.format(f))
		savepath = 'cmpth_checkpoints/exp23/checkpoints/fold{}/'.format(f)+ 'fold{}_'.format(f) + datetime.now().strftime("%d_%m_%Y___%H_%M_%S")+'.pth'
		torch.save(model, savepath)
			


wpath = 'cmpth_checkpoints/exp23/logs/fold{}/'.format(f)+'trainlossrecord.npy'
trainlossrecord = np.array(trainlossrecord)
np.save(wpath, trainlossrecord)

wpath = 'cmpth_checkpoints/exp23/logs/fold{}/'.format(f)+'vallossrecord.npy'
vallossrecord = np.array(vallossrecord)
np.save(wpath, vallossrecord)

wpath = 'cmpth_checkpoints/exp23/logs/fold{}/'.format(f)+'valaccrecord.npy'
